...
Battle pets may also earn experience that eventually helps them to level up.  When a battle pet levels up (currently at every 1000 experience points), their battle characteristics also get a small boost, improving the pet's chance of winning battles.
...
Battles designated as play for keeps, give the winner the loser's pet at their completion.
...
An admin can add more arenas to the battle pets manager in order to allow new arenas to be added to the Arena Empire.
...
Quick run:
	In BattlePets
		Login as the user:
			...
			Note that Training exercises do not show up on your Win/Loss record, ony real battles with real battle pets.
